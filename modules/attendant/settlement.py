...
def render_closing_reading_editor(row: dict, key_prefix: str):
    """
    Settlement page me wahi closing-reading logic use hoga jo Sale Approval page me use ho raha hai.
    Isse reading/difference dono screens par same result dikhayenge.
    """
    st.markdown("<div class='step-title'>Manager Closing Reading</div>", unsafe_allow_html=True)

    settlement_id = row.get("id")
    settlement, assignments, load_error = get_closing_assignments_for_approval(settlement_id)
    ...
    for idx, assignment in enumerate(assignments):
        ...
        with c4:
            closing = st.number_input(
                ...
                step=0.01,
                format="%.2f",
                key=f"settlement_closing_{key_prefix}_{idx}_{assignment_id}",
            )

        closing_inputs[assignment_id] = closing

    calc_rows, meter_total, error = preview_closing_calculation(settlement_id, closing_inputs)
    ...
    st.markdown("<div class='step-title'>Live Reading Difference</div>", unsafe_allow_html=True)
    ...
    if st.button("Save Closing Readings", type="primary", key=f"save_closing_{key_prefix}", use_container_width=True):
        updated, save_error = save_closing_for_approval(
        ...
        if updated:
            st.success("Closing readings saved. Settlement meter sale/difference updated.")
            st.rerun()
        else:
