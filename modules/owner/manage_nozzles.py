...
def show_nozzle_list():
    ...
    st.dataframe(nozzles, use_container_width=True)
...
def create_nozzle_form():
    ...
    with st.form("create_nozzle_form"):
        ...
        current_reading = st.number_input("Current Reading", min_value=0.0, step=0.01, format="%.2f")
...
def edit_nozzle_form():
    ...
    nozzle_labels = {
        f"{n.get('id')} | {n.get('nozzle_name')} | {n.get('fuel_type')}": n
        for n in nozzles
    }
    ...
    with st.form("edit_nozzle_form"):
        nozzle_name = st.text_input("Nozzle Name", value=selected_nozzle.get("nozzle_name") or "")
        fuel_type = st.selectbox("Fuel Type", VALID_FUEL_TYPES, index=fuel_index)
        ...
        is_active = st.checkbox("Active", value=bool(selected_nozzle.get("is_active")))
