...
def get_active_tanks():
    try:
        return get_supabase_client().table("stock_tanks").select("*").eq("is_active", True).order("fuel_type").execute().data or []
    except Exception as e:
        print("get_active_tanks", e); return []
...
def create_or_update_tank(fuel_type, tank_name, capacity_liters, opening_stock, current_stock, created_by):
    ...
    payload = {
        "fuel_type": fuel_type, "tank_name": tank_name or f"{fuel_type.title()} Tank",
        "capacity_liters": _f(capacity_liters), "opening_stock": _f(opening_stock),
        "current_stock": _f(current_stock), "is_active": True,
...
def create_oil_company_ledger(oil_company, txn_type, amount, reference_no, fuel_type=None, quantity_liters=0, created_by=None):
    ...
    payload = {"date": _today(), "oil_company": oil_company, "type": txn_type, "fuel_type": fuel_type,
               "quantity_liters": _f(quantity_liters), "amount": _f(amount), "reference_no": reference_no,
               "created_by": created_by, "created_at": _now()}
    try:
...
def create_fuel_inward(data):
    fuel_type = data.get("fuel_type"); qty = _f(data.get("quantity_liters")); rate = _f(data.get("rate"))
    if fuel_type not in FUEL_TYPES: return None, "Invalid fuel type."
    ...
    payload = {"date": data.get("date") or _today(), "oil_company": data.get("oil_company"),
               "invoice_no": data.get("invoice_no"), "tanker_no": data.get("tanker_no"),
               "fuel_type": fuel_type, "quantity_liters": qty, "rate": rate, "total_amount": total,
    ...
    try:
        ...
        if inward:
            ...
            create_oil_company_ledger(data.get("oil_company"), "inward", total, data.get("invoice_no"), fuel_type, qty, data.get("created_by"))
        return inward, None
    except Exception as e:
...
def create_daily_testing(data):
    fuel_type = data.get("fuel_type"); liters = _f(data.get("testing_liters"))
    if fuel_type not in FUEL_TYPES: return None, "Invalid fuel type."
    tank = get_tank_by_fuel(fuel_type)
    if not tank: return None, "Create tank first."
    payload = {"date": data.get("date") or _today(), "fuel_type": fuel_type,
               "density": _f(data.get("density")), "temperature": _f(data.get("temperature")),
               "testing_liters": liters, "result": data.get("result"), "remark": data.get("remark"),
               "tested_by": data.get("tested_by"), "created_at": _now()}
    try:
        r = get_supabase_client().table("daily_testing").insert(payload).execute()
        row = r.data[0] if r.data else None
        if row and liters > 0:
            update_tank_stock(fuel_type, max(0, round(_f(tank.get("current_stock")) - liters, 2)))
        return row, None
    except Exception as e:
...
def get_daily_testing(entry_date=None):
    try:
        q = get_supabase_client().table("daily_testing").select("*")
        if entry_date: q = q.eq("date", entry_date)
        return q.order("created_at", desc=True).execute().data or []
    except Exception as e:
...
def _sum_by_fuel(rows, key):
    ...
    for r in rows:
        ft = r.get("fuel_type")
        if ft in total: total[ft] += _f(r.get(key))
    return {k: round(v, 2) for k, v in total.items()}

def get_inward_totals(entry_date=None): return _sum_by_fuel(get_fuel_inward(entry_date), "quantity_liters")
def get_testing_totals(entry_date=None): return _sum_by_fuel(get_daily_testing(entry_date), "testing_liters")
    ...
    try:
        ...
        for s in r.data or []:
            for row in (s.get("nozzle_readings") or []):
                ...
                if ft in total: total[ft] += _f(row.get("actual_liters"))
    except Exception as e:
...
def get_stock_summary(entry_date=None):
    entry_date = entry_date or _today()
    tanks = get_all_tanks(); inward = get_inward_totals(entry_date); testing = get_testing_totals(entry_date); sales = get_sale_liters_from_settlements(entry_date)
    out = {}
    for ft in FUEL_TYPES:
        ...
        expected = round(opening + _f(inward.get(ft)) - _f(sales.get(ft)) - _f(testing.get(ft)), 2)
        out[ft] = {"fuel_type": ft, "tank_name": tank.get("tank_name") if tank else None,
                   "capacity_liters": _f(tank.get("capacity_liters")) if tank else 0.0,
                   "opening_stock": opening, "inward_stock": _f(inward.get(ft)), "sale_liters": _f(sales.get(ft)),
                   "testing_liters": _f(testing.get(ft)), "expected_closing_stock": expected,
                   "current_stock": current, "stock_difference": round(current - expected, 2)}
    return out

def save_stock_closing(data):
    fuel_type = data.get("fuel_type"); physical = _f(data.get("physical_stock"))
    if fuel_type not in FUEL_TYPES: return None, "Invalid fuel type."
    summary = get_stock_summary(data.get("date") or _today())[fuel_type]
    expected = _f(summary.get("expected_closing_stock")); diff = round(physical - expected, 2)
    payload = {"date": data.get("date") or _today(), "fuel_type": fuel_type,
               "expected_stock": expected, "physical_stock": physical, "difference": diff,
               "remark": data.get("remark"), "created_by": data.get("created_by"), "created_at": _now()}
    try:
        r = get_supabase_client().table("stock_closing").insert(payload).execute()
        row = r.data[0] if r.data else None
        if row: update_tank_stock(fuel_type, physical)
        return row, None
    except Exception as e:
...
def create_oil_company_payment(oil_company, amount, reference_no, created_by):
    if _f(amount) <= 0: return None, "Payment amount required."
    ledger, err = create_oil_company_ledger(oil_company, "payment", amount, reference_no, created_by=created_by)
    payload = {"date": _today(), "oil_company": oil_company, "amount": _f(amount), "reference_no": reference_no, "created_by": created_by, "created_at": _now()}
    try:
...
def get_oil_company_ledger(oil_company=None):
    try:
        ...
        if oil_company: q = q.eq("oil_company", oil_company)
        return q.order("created_at", desc=True).execute().data or []
    except Exception as e:
        print("get_oil_company_ledger", e); return []

def get_oil_company_summary():
    rows = get_oil_company_ledger(); summary = {}
    for r in rows:
        c = r.get("oil_company") or "Unknown"
        summary.setdefault(c, {"Oil Company": c, "Inward Amount": 0.0, "Payment Made": 0.0, "Outstanding": 0.0})
        if r.get("type") == "inward": summary[c]["Inward Amount"] += _f(r.get("amount"))
        elif r.get("type") == "payment": summary[c]["Payment Made"] += _f(r.get("amount"))
