...
def get_approved_settlements(entry_date=None):
    try:
        ...
        print("get_approved_settlements", e); return []
...
def _get_approved_expense_total(entry_date=None, payment_mode=None):
    try:
...
def create_cash_deposit(amount, bank_name, reference_no, deposited_by, deposit_date=None, note=None):
    if _f(amount) <= 0: return None, "Cash deposit amount must be greater than 0."
    ...
    try:
        ...
        print("create_cash_deposit", e); return None, str(e)
...
def get_cash_deposits(entry_date=None):
    try:
        ...
        print("get_cash_deposits", e); return []
...
def create_paytm_settlement(amount, bank_name, reference_no, settled_by, settlement_date=None, note=None):
    if _f(amount) <= 0: return None, "Paytm settled amount must be greater than 0."
    ...
    try:
        ...
        print("create_paytm_settlement", e); return None, str(e)
...
def get_paytm_settlements(entry_date=None):
    try:
        ...
        print("get_paytm_settlements", e); return []
...
def create_ccms_settlement(amount, bank_name, reference_no, settled_by, settlement_date=None, note=None):
    if _f(amount) <= 0: return None, "CCMS received amount must be greater than 0."
    payload = {"date": settlement_date or _today(), "amount": _f(amount), "bank_name": bank_name, "reference_no": reference_no, "note": note, "settled_by": settled_by, "created_at": _now()}
    try:
        r = get_supabase_client().table("ccms_settlements").insert(payload).execute()
        return (r.data[0] if r.data else None), None
    except Exception as e:
        print("create_ccms_settlement", e); return None, str(e)
...
def get_ccms_settlements(entry_date=None):
    try:
        ...
        print("get_ccms_settlements", e); return []
...
def get_daily_money_summary(entry_date=None):
    ...
    cash_exp = _get_approved_expense_total(entry_date, "cash")
    bank_exp = _get_approved_expense_total(entry_date, "bank")
    upi_exp = _get_approved_expense_total(entry_date, "upi")
    paytm_exp = _get_approved_expense_total(entry_date, "paytm")
    ...
    return {
        ...
        "paytm_expense": paytm_exp,
        "paytm_pending": round(p["paytm"] - paytm_set - paytm_exp, 2),
