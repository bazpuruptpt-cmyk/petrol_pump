...
def approve_expense(expense_id, approved_by, note=None):
    ...
    if exp.get("status") == "approved":
        return None, "Expense already approved."
    return update_expense_status(expense_id, "approved", approved_by, note)

def reject_expense(expense_id, approved_by, note=None):
    return update_expense_status(expense_id, "rejected", approved_by, note)

def hold_expense(expense_id, approved_by, note=None):
    return update_expense_status(expense_id, "hold", approved_by, note)

def reopen_expense(expense_id, approved_by, note=None):
    return update_expense_status(expense_id, "reopened", approved_by, note)
...
def _money(entry_date=None):
    try:
        ...
        return {"total_sale": 0, "cash_sale": 0, "paytm_sale": 0, "ccms_sale": 0, "credit_sale": 0}
...
def _purchase(entry_date=None):
    try:
        ...
        for r in rows:
            ...
            if r.get("fuel_type") == "petrol":
                out["petrol_purchase"] += amt
            if r.get("fuel_type") == "diesel":
                out["diesel_purchase"] += amt
